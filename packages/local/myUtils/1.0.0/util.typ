#let kinto(lr: true, body) = {
  body = if body.has("text") { body.text }
  if type(body) != str { return none }
  if lr == true { h(1fr, weak: true) }
  let i = 0
  for c in body.clusters() {
    let l = body.clusters().len()
    i += 1
    c
    if i < l { h(1fr, weak: true) }
  }
  if lr == true { h(1fr, weak: true) }
}
...
#let fmt-jd(
  ...
  let date = if td == array { date } else if td == str { date.split(regex("[ .-]")) }
  let publish = if tp == array { publish } else if tp == str { publish.split(regex("[ .-]")) }
